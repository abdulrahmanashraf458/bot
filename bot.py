...
TOKEN = "7426204733:AAFPxKvpxB78V4mF2Q7bdBOiEsvLeCDAN84"
    ...
    try:
        if os.path.exists('data.json'):
            with open('data.json', 'r', encoding='utf-8') as file:
                ...
                data['premium_users'] = {int(k): datetime.fromisoformat(v) for k, v in data.get('premium_users', {}).items()}
                data['premium_plus_users'] = {int(k): datetime.fromisoformat(v) for k, v in data.get('premium_plus_users', {}).items()}
...
def save_data():
    with open('data.json', 'w', encoding='utf-8') as file:
        ...
        data_to_save['premium_users'] = {k: v.isoformat() for k, v in data_to_save['premium_users'].items()}
        data_to_save['premium_plus_users'] = {k: v.isoformat() for k, v in data_to_save['premium_plus_users'].items()}
...
def check_daily_limit(user_id):
    if is_owner(user_id):
        return 0
    if user_id in data['premium_plus_users'] and data['unlimited_access_premium_plus']:
        return 0
    limit = data['premium_plus_daily_limit'] if user_id in data['premium_plus_users'] else (data['premium_daily_limit'] if user_id in data['premium_users'] else data['daily_limit'])
...
async def start(update: Update, context: CallbackContext):
    ...
    await update.message.reply_text("Hello! Use /menu to get an account or /feedbackmenu to report an issue or give feedback.")
...
async def handle_document(update: Update, context: CallbackContext):
    ...
    if context.user_data.get('awaiting_upload'):
        ...
        await update.message.reply_text("Please specify the section name first using /uploadsection.")
...
async def show_menu(update: Update, context: CallbackContext):
    ...
    if user_id in data['premium_plus_users'] or user_id in data['premium_users']:
